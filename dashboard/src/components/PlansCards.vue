<template>
	<div class="@container" v-if="plans.length">
		<div class="grid grid-cols-2 gap-3 @xl:grid-cols-3">
			<button v-for="(plan, i) in plans" :key="plan.name"
				class="flex flex-col overflow-hidden rounded-md border text-left hover:bg-gray-50" :class="[
				...
				<div class="h-16 w-full border-b p-3" :class="[
					...
					<div class="flex items-center justify-between">
						<div class="flex w-full items-center text-lg">
							...
							<Tooltip text="Product support included">
								<lucide-badge-check class="ml-1 h-4 w-6 text-gray-600" v-if="plan.support_included" />
							</Tooltip>
						</div>
					</div>
